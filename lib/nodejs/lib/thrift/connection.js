...
var isFunction = value => value && (Object.prototype.toString.call(value) === "[object Function]" || "function" === typeof value || value instanceof Function);
...
var Connection = exports.Connection = function(stream, options) {
  ...
  this.onTerminated = isFunction(options.onTerminated) ? options.onTerminated : null;
...
Connection.prototype.connection_gone = function () {
  ...
  if (this.max_attempts && this.attempts >= this.max_attempts) {
    this.retry_timer = null;
    let errMsg = "thrift: Couldn't get thrift connection after " + this.max_attempts + " attempts."
    log.error(errMsg);
    self.emit("close");
    if(self.onTerminated){
        self.onTerminated(new Error("[ERROR] beyond max attempts; " + errMsg));
    }
    return;
  }
  ...
  this.retry_timer = setTimeout(function () {
    ...
    if (self.connect_timeout && self.retry_totaltime >= self.connect_timeout) {
       ...
       let errMsg = "thrift: Couldn't get thrift connection after " + self.retry_totaltime + "ms.";
       log.error(errMsg);
       self.emit("close");
       if(self.onTerminated){
            self.onTerminated(new Error("[ERROR] beyond max retry_totaltime; " + errMsg));
       }
       return;
    }
